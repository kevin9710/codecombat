...
setupExpressMiddleware = (app) ->
  ...
  if config.buildInfo.sha isnt '17fa3411a9094d3a11dc3a575970416d015c7bcf' and config.isProduction
    app.use("/" + config.buildInfo.sha, express.static(public_path, maxAge: '1y'))
  else
    app.use("/" + config.buildInfo.sha, express.static(public_path, maxAge: 0))  # CloudFlare overrides maxAge, and we don't want local development caching.
